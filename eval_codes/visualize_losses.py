...
def plot_losses(opt_paths, a, plot_detail_loss=False):
    ...
    for opt_path in opt_paths:
        ...
        # Plot pretrain loss
        plt.figure()
        plt.plot(train_log['step'], smoothing(train_log['loss_G'], a), label=f'train_loss_G', alpha=0.6)
        plt.plot(test_log['step'], test_log['loss_G'], label=f'test_loss_G', alpha=0.6)
        if plot_detail_loss:
            plt.plot(train_log['step'], smoothing(train_log['l1loss'], a), label=f'train_l1loss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['ploss'], a), label=f'train_ploss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['sloss'],  a), label=f'train_sloss', alpha=0.6)
        # plt.ylim(top=1.0)
        plt.xlabel('Steps')
        ...
        plt.figure()
        plt.plot(train_log['step'], smoothing(train_log['loss_G'], a), label=f'train_loss_G', alpha=0.6)
        plt.plot(train_log['step'], smoothing(train_log['loss_D'], a), label=f'train_loss_D', alpha=0.6)
        if plot_detail_loss:
            plt.plot(train_log['step'], smoothing(train_log['advloss'], a), label=f'train_advloss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['l1loss'], a), label=f'train_l1loss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['ploss'], a), label=f'train_ploss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['sloss'], a), label=f'train_sloss', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['loss_D_real'], a), label=f'train_loss_D_real', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['loss_D_fake'], a), label=f'train_loss_D_fake', alpha=0.6)
            plt.plot(train_log['step'], smoothing(train_log['gp'], a), label=f'train_gp', alpha=0.6)
        # plt.ylim(top=1.0)
        plt.xlabel('Steps')
        plt.ylabel('Loss Value')
        plt.legend()
